...
## Описание реализации
Есть два основных класса: QuadrantSeparator и StakeHolderAnalyzer

Первый поддерживает фильтрацию точек по координатам (в нащем случае 
используется фильтрация: x и y больше половины числа точек)

Второй содержит обработку датасетов стейкхолдеров. В частности:

Считывание матрицы параметра из файла

Конвертация в координаты и получение самых важных, используя QuadrantSeparator

Матрица параметра хранится в record StakeHoldersRelations. Он должен 
поддерживать различные агрегации (но в нашей задаче нужен только sum)

Класс Main - точка входа через командную строку. Принимает пути до interest.txt и influence.txt и путь ответа. Можно задать только все три 
аргумента, два первых или ни одного. Если ни один 
аргумент не задан - 
читает interest.txt, influence.txt из директории запуска программы, 
пишет в result.txt в директрию запуска программы. 

## Инструкция по сборке и запуску решения
Если находитесь в директории проекта:

```java -jar ./target/school2025-test-task-1.0-SNAPSHOT.jar [PATH TO INTEREST] [PATH TO INFLUENCE] [PATH TO OUTPUT]```

Иначе прописать вместо . путь до директрии проекта

В репозиторий добавлены тестовые interest и influence, поэтому можно вызвать
```java -jar ./target/school2025-test-task-1.0-SNAPSHOT.jar```

jar уже есть в репозитории, но его можно собрать по-новой, находясь в 
директории проекта:
...
Или открыть в intelliJ и потыкать кнопочки :)