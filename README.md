...
Далее программа считывает списки заинтересованных сторон и матрицу интересов из файла `interest.txt`, а также матрицу влияния из файла `influence.txt`.Путем сложения значений в каждой строке матрицы определяются общие ранги всех заинтересованных сторон. Самые важные заинтересованные стороны определяются как те, которые находятся в правом верхнем квадранте матрицы, включая стороны на его границах. Для важных сторон устанавливается ранг, в то время как для остальных ранг равен 0. На основании рангов выбираются индексы важных сторон, которые затем записываются в выходной файл `result.txt`.
