...
## Описание реализации
Реализована программа, которая определяет наиболее важных стейкхолдеров и записывает их в result.txt. 

В своей реализации я учел, что стейкхолдеры удовлетворяют условиям задачи, если их ранги (суммы в матрицах попарного сравнения) больше среднего ранга. Только так они находятся в правом верхнем квадранте. Программа сначала считает ранг каждого стейкхолдера в каждом файле, а после сравнивает эти ранги со средними значениями в файле. На вывод подаются все стейкхолдеры, находящиеся в правом верхнем квадранте. При этом стейкхолдеры, находящиеся на границах этого квадранта, не учитываются.

## Инструкция по сборке и запуску решения
Запустите программу, предварительно внеся изменения в файлы interest.txt и influence.txt. Результат выполнения программы будет находиться в result.txt. 
