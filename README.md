...
## Описание реализации
Приложение может запускаться с 0 или 3 параметрами командной строки. Если запуск с 0 параметров, то файлы "interest.txt", "influence.txt" и "result.txt" должны находится в текущей директории,
при запуске с 3 аргументами необходимо указать пути до соответствующих файлов.
После чтения данных из файлов, приложение вычисляет итоговый ранг по интересу и влиянию для каждого стейкхолдера при помощи функции ```calculateRanks```.
После этого определются те стейкхолдеры, которые находятся в правом верхнем квадранте и затем, они записываются в файл "result.txt".
## Инструкция по сборке и запуску решения
Приложение собирается при помощи gradle.
