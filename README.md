...
## Автор решения
Юнеев Руслан Закирович

## Описание реализации
Утилитный enum класс `Definer` в пакете `utils` предоставляет объекты с 3 аттрибутами (путь к файлу с рангами интереса, путь к файлу с рангами влияние, путь к файлу, куда будут записаны самые важные стейкхолдеры). В нашем случае мы имеем `TOPMOST_STAKEHOLDER_DEFINER`. Взаимодействие с API класса происходит через публичный метод `writeTopmostStakeholdersToFile()`, не принимающий аргументов, который внутри использует 2 приватных метода `calculateRank(String filePath)`, `defineTopmostStakeholders()`, подсчитывающий ранг интереса/влияния и находящий самых важный стейкхолдеров соответственно. Может выбрасываться исключение `InvalidFilePatternException` из пакета `exceptions`, если нарушен шаблон заполнения файлов `interest.txt` и `influence.txt`.

## Инструкция по сборке и запуску решения
### 1. Скачать репозиторий.
### 2. Открыть проект в среде разработки (например, IntelliJ IDEA).
### 3. Заполнить/изменить согласно шаблону, данному в условии, файлы `interest.txt`, `influence.txt` в папке `src/main/resources` (или удалить и добавить свои).
### 4. Запустить файл `App.java`.
### 5. Посмотреть результат в файле `result.txt` в папке `src/main/resources`.