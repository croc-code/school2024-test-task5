...
## Автор решения
Ковалев Данил Владимирович
danil.kovaleow@yandex.ru

## Описание реализации
Решение реализовано на языке Python 3.12. Для запуска требуется два файла interest.txt и influence.txt (которые даны в условии задачи). В результате работы программы появится файл с результатом (result.txt). В реализации использовались только стандартные инструменты языка.
