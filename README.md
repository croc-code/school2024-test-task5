...
Основное решение было резализованно на чистом `Python` без использования библиотек.
1. Считывание матрицы из файла c заменой '_' на 0.
2. Подсчет рангов для каждого пользователя через суммирование строк внутри матрицы. Сохранение массива с именами стейкхолдеров.
3. Подсчет "координат" в осях interests-influence (time complexity - O(n), где n - количество стейкхолдеров). Подсчет производится с использованием словаря, в котором изначально всех стейкхолдеров мы считаем лучшими, и далее проходясь по массивам с рангами, если они располагаются не в правом верхнем квадрате, то мы меняет их значение в словаре, тем самам они перестают быть лучшими стейкхолдерами. В итоге получается словарь, в котором значение True соответствует лучшеим стейкхолдерам, а False — остальным.
4. Выбор лучших стейкхолдеров, с помощью прохода по выше созданному словарю.

Все основное решение находится в функции `get_most_valuable_stakeholders()`

Также для визуального тестирования была написана функция `show_interest_influence_plot()`, использующая `Matplotlib`. 
Данная фукнция отрисовывает "координаты" стейкхолдеров в осях interests-influence

## Инструкция по сборке и запуску решения

Для запуска основного задания:
1. Файлы interest.txt и influence.txt должны быть расположены в той же директории, что и `main.py`.
1. Запустить файл `main.py`

Для запуска визуализации "координат" стейкхолдеров в осях interests-influence (этого не требовалось по заданию):
1. Установить пакет Matplotlib (`pip install matplotlib`)
2. Запустить файл `show_plot.py`
