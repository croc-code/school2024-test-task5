...
все файлы с кодом проекта перенес в репозиторий форка, соотвественно ниже по схеме показано где они должны быть.
...
│   ├── main
│   │   ├── java
│   │   │   └── (исходные файлы .java, такие как Main.java)
│   │   └── resources
│   │       └── (тут лежит файл interest.txt и result.txt)
│   └── test
│       ├── java
│       │   └── (тестовые исходные файлы .java)
│       └── resources
│           └── (ресурсы для тестов)
└── pom.xml (или другие файлы конфигурации)
