...
## Автор решения
Питер Александр Витальевич (Qefy)


Почта: siralexpeter@mail.ru


Телеграм: @floor_is_lava

## Описание реализации
При реализации были использованы только встроенные функции python.
Написан один класс для удобного представления входных матриц.
Также написан ещё один класс, в котором написан статический метод, отвечающий 
за нахождения самых важных стейкхолдеров.


Так же применена аннотации типов, что может помочь при внесении изменений в код, 
т.к. сейчас многие IDE подсвечивают типы переменных python. 


## Инструкция по сборке и запуску решения
Для запуска желательно использовать новые версии python, чтобы не возникали возможные ошибки из-за 
использования аннотации типов.

### До запуска кода
Отредактировать или заменить на свои файлы `influence.txt` и `influence.txt`.
### Запуск кода
Перейти в дирректорию solution.
```
cd solution
```
Запустить код:
...
Посмотреть результат работы кода можно в текстовом файле `result.txt`,
который создаётся или перезаписывается в дирректории solution.


В терминал выводятся входные данные, представленные в виде объектов класса Table  
